...
\usepackage{listings}
\usepackage{amsmath}
% NOTE: ceurart doesn't reliably support the float package's [H] placement.
% Keep floats stable using standard placements + FloatBarrier instead.
% \usepackage{float}
\usepackage{caption} % \captionof for non-floats
...
% Don't let floats drift into later sections/appendix
\let\OldAppendix\appendix
\renewcommand{\appendix}{\FloatBarrier\OldAppendix}

% Keep floats from drifting too far (typical "more inline" behavior)
\makeatletter
...
\captionof{figure}{Score progression across training steps for Zagreus-0.4B-fra (data from
...
\captionof{figure}{Score progression across checkpoints for Zagreus-0.4B-por (data from
...
[tbp] = lascia scegliere tra top/bottom/page-of-floats (di solito la migliore)

Quindi, in generale:

table* / figure* → [t] o [tbp] (due colonne / larghezza pagina)

table / figure → [tbp]

2) (Opzionale ma consigliato) Usa placeins per “bloccare” i float per sezione
Se vuoi leggibilità “ogni sezione chiude i suoi float”, aggiungi:

\usepackage[section]{placeins}
Questo inserisce automaticamente un \FloatBarrier a fine sezione: i float non “scappano” troppo avanti.

3) (Opzionale) Migliora la gestione float con dblfloatfix (utile per figure e table a 2 colonne)
Con classi a due colonne spesso conviene:

\usepackage{dblfloatfix}
(gestisce meglio i float “*” e i casi in cui LaTeX è troppo conservativo)

Patch pronta da applicare al tuo file
A) Preambolo
Hai:

\usepackage{float}   % [H] placement — keeps figures exactly where declared
Se vuoi che non siano inline, fai una di queste:

Opzione 1 (semplice): lascia float ma smetti di usare [H]
Opzione 2 (più pulita): rimuovi proprio float per evitare tentazioni

Io farei così:

% \usepackage{float}   % remove if you don't need [H]
\usepackage[section]{placeins} % keep floats within sections (optional but great)
\usepackage{dblfloatfix}       % better handling for two-column floats (optional)
B) Sostituzioni rapide nel corpo
ogni \begin{table*}[H] → \begin{table*}[t] (o [tbp])

ogni \begin{table}[H] → \begin{table}[tbp]

ogni \begin{figure}[H] → \begin{figure}[tbp] (o [t])

ogni \begin{figure*}[H] → \begin{figure*}[t]

Esempio reale dal tuo snippet:
...
...
\end{table*}
Impostazioni “più leggibili” (se vuoi spingere ancora di più)
Se noti che LaTeX rimanda troppe figure/tabelle in fondo, puoi aggiungere (con moderazione) queste righe nel preambolo:

\renewcommand{\topfraction}{0.9}
\renewcommand{\bottomfraction}{0.8}
\renewcommand{\textfraction}{0.07}
\renewcommand{\floatpagefraction}{0.85}
Servono a dire a LaTeX: “puoi mettere più float per pagina”.

Regola pratica per il tuo paper
Usa [t] per i grafici principali (più leggibile: grafico vicino al punto in cui lo citi, ma non spezza il testo).

Usa [tbp] per tabelle secondarie o lunghe.

Usa table* / figure* solo quando serve davvero la larghezza pagina (altrimenti i float “*” sono più difficili da piazzare).

Se vuoi, incollami solo l’elenco delle tue \begin{table...} / \begin{figure...} (anche senza contenuto) e ti riscrivo al volo tutte le opzioni di placement in modo coerente (es. tutte le “main results” in alto pagina, tutte le “commands” in basso o dopo \subsection).


Hai detto:
dammi il file corretto ma io voglio che le tabelle e immagini siano proprio nella posizione in cui sono citate non voglio che latex le metta dove vuole hai capito?

ChatGPT ha detto:

Ragionato per 16s
Sì, ho capito: vuoi che tabelle e figure compaiano esattamente nel punto in cui le metti nel sorgente (cioè subito dopo la frase che le cita), senza che LaTeX le sposti.

Per ottenere questo in modo robusto con ceurart:

Usa sempre [H] (serve \usepackage{float}).

Evita table* / figure*: con alcune classi possono comunque comportarsi “da float speciale”. Se non stai usando twocolumn, non ti serve *.

Se vuoi larghezza pagina intera, usa \resizebox{\linewidth}{!}{...} (come già fai) oppure \makebox[\linewidth]{...}.

(Opzionale ma utile) \FloatBarrier per impedire accumuli di float (ma con [H] di solito non serve).

Qui sotto ti lascio il file corretto con questi cambiamenti:

mantengo float e [H]

sostituisco tutti i table* → table e figure* → figure

lascio \resizebox{\linewidth}{!}{...} dove serve

Nota: se in futuro abiliti twocolumn, l’“esattamente qui” diventa intrinsecamente più fragile (perché il layout a due colonne impone vincoli). Ma nel tuo file attuale twocolumn è commentato, quindi va benissimo.
...
\usepackage{listings}
\usepackage{amsmath}
\usepackage{float} % enables [H] to force exact placement
% \usepackage[section]{placeins} % optional: \FloatBarrier per section (not necessary with [H])
...
% NOTE: table* -> table, [H] keeps it exactly here
\begin{table}[H]
\caption{Checkpoint progression for Zagreus-0.4B-ita-base. Metrics: MMLU (acc),
         HellaSwag (acc\_norm), ARC (acc\_norm).}
\label{tab:zagreus-ita}
...
\end{document}
Importante (super pratico)
Se vuoi che sia “proprio dove è citata”, la regola d’oro è:

scrivi la frase che cita (Table~\ref{...})

subito dopo metti \begin{table}[H] ... \end{table}

uguale per figure.

Se mi incolli la versione completa del tuo file (non tronca) te la restituisco “pulita” al 100% senza i ... unchanged ... e con tutti i blocchi al loro posto, identica ma già sistemata.


Hai detto:
this is the latex all:
...
ChatGPT ha detto:
Perfetto. Il tuo file è già quasi corretto per il requisito “esattamente dove lo metto”: stai già usando \usepackage{float} + [H]. L’unico punto che può ancora crearti comportamento “strano” in ceurart è l’uso di table* (e in generale gli ambienti *), che in alcune classi sono trattati in modo speciale.

Quindi ti restituisco il file completo corretto facendo SOLO queste modifiche (coerenti con quello che hai chiesto):

tutti i \begin{table*}[H] ... \end{table*} diventano \begin{table}[H] ... \end{table}

tutto il resto resta identico

dove serve la larghezza pagina, aggiungo \resizebox{\linewidth}{!}{...} (solo nel caso in cui la tabella rischi overflow; tu già lo fai in 2 tabelle grandi, quindi lo lascio com’è)

le figure restano figure[H] (già ok)
...
% FIX: table* -> table to avoid special float handling in some classes
\begin{table}[H]
\caption{Checkpoint progression for Zagreus-0.4B-ita-base. Metrics: MMLU (acc),
         HellaSwag (acc\_norm), ARC (acc\_norm).}
\label{tab:zagreus-ita}
...
\end{document}
Perché questa è la versione giusta per “NON spostare nulla”
[H] forza davvero la posizione “qui”.

Eliminando table* eviti il comportamento speciale dei float “wide” in certe classi (che è la cosa più comune che fa “scappare” le tabelle anche quando metti [H]).

Se mi dici se abiliterai twocolumn in futuro, ti do anche la variante “a due colonne” che mantiene il più possibile l’“esattamente qui” (ma lì ci sono compromessi reali di impaginazione).


Hai detto:
scusa ma ancora le tabelle e immagine sono in appendix non sono inline come ti ho chiesto per favore impegnati 

ChatGPT ha detto:

Ragionato per 14s
Hai ragione: se ti finiscono tutte in fondo/appendix, vuol dire che ceurart sta comunque trattando table/figure come float e le “deferisce” (anche con [H] in certi casi/impaginazioni).

Se vuoi la garanzia assoluta “inline esattamente qui”, la soluzione robusta è: NON usare proprio gli ambienti float table/figure, ma usare caption non-floating con \captionof{table} / \captionof{figure}. Così LaTeX non può spostarle, perché non sono float.

Sotto trovi il file completo corretto:

aggiungo \usepackage{caption}

sostituisco tutte le \begin{table}...\end{table} / \begin{figure}...\end{figure} con blocchi non-floating \captionof{...} + \label{...}.

Nota: lascia pure \usepackage{float} se vuoi, ma con questa soluzione non serve più.
...
% --- IMPORTANT: non-floating captions (forces inline placement) ---
\usepackage{caption} % provides \captionof{table} / \captionof{figure}
...
\label{sec:motivation}
% ================================================================
% ... (unchanged text up to the first table/figure) ...
...
% ===================== INLINE (NON-FLOATING) TABLE =====================
\par\medskip
\begin{center}
\captionof{table}{Checkpoint progression for Zagreus-0.4B-ita-base. Metrics: MMLU (acc),
         HellaSwag (acc\_norm), ARC (acc\_norm).}
\label{tab:zagreus-ita}
\begin{tabular}{lcccc}
\toprule
...
% ===================== INLINE (NON-FLOATING) FIGURE =====================
\par\medskip
\begin{center}
\includegraphics[width=\linewidth]{images/zagreus-ita.png}
\captionof{figure}{Score progression across checkpoints for Zagreus-0.4B-ita-base (data from
         Table~\ref{tab:zagreus-ita}).}
\label{fig:zagreus-ita}
\end{center}
\par\medskip
...
% ===================== INLINE TABLE (NON-FLOATING) =====================
\par\medskip
\begin{center}
\captionof{table}{EVALITA evaluation from FBK --- Zagreus-0.4B-ita-base, per-task
         breakdown~\cite{evalita2023}.}
\label{tab:evalita-fbk}
\begin{tabular}{l l c}
\toprule
...
\par\medskip
\begin{center}
\captionof{table}{Zagreus-0.4B-spa-base results across training steps.}
\label{tab:zagreus-spa}
\begin{tabular}{lcccc}
\toprule
Steps & mmlu\_es & arc\_es & hellaswag\_es & Average \\
...
\begin{center}
\includegraphics[width=\linewidth]{images/zagreus-spa.png}
\captionof{figure}{Score progression across training steps for Zagreus-0.4B-spa-base (data from
         Table~\ref{tab:zagreus-spa}).}
\label{fig:zagreus-spa}
\end{center}
\par\medskip
...
\par\medskip
\begin{center}
\captionof{table}{Zagreus-0.4B-fra results across training steps.}
\label{tab:zagreus-fra}
\begin{tabular}{lcccc}
\toprule
Steps & m\_mmlu\_fr & arc\_fr & hellaswag\_fr & Average \\
...
\par\medskip
\begin{center}
\captionof{table}{Zagreus-0.4B-por results across checkpoints.}
\label{tab:zagreus-por}
\begin{tabular}{lcccc}
\toprule
Checkpoint & ARC & HellaSwag & MMLU & Media \\
...
% INLINE wide table: keep resizebox but non-floating
\par\medskip
\begin{center}
\captionof{table}{Portuguese leaderboard-style evaluation
         (lm-evaluation-harness-pt~\cite{eduagarcia-lmevalpt}).
         Zagreus 483k ranks first overall.}
\label{tab:por-garcia}
\resizebox{\linewidth}{!}{%
...
\begin{center}
\includegraphics[width=\linewidth]{images/por-garcia.png}
\captionof{figure}{Portuguese leaderboard comparison between Zagreus checkpoints and Qwen3-0.6B-Base
         (data from Table~\ref{tab:por-garcia}).}
\label{fig:por-garcia}
\end{center}
\par\medskip
...
% INLINE big comparison table
\par\medskip
\begin{center}
\captionof{table}{English/Italian benchmark comparison across post-trained and baseline models.
         Benchmarks: IFEval~\cite{zhou2023ifeval}, ARC~\cite{clark2018arc},
         HellaSwag~(HS)~\cite{zellers2019hellaswag}, MMLU~\cite{hendrycks2021mmlu}.}
\label{tab:nesso-main}
\resizebox{\linewidth}{!}{%
...
\begin{center}
\includegraphics[width=\linewidth]{images/nesso-all.png}
\captionof{figure}{Overall benchmark comparison (English + Italian average) across all Nesso and
         baseline models. Data from Table~\ref{tab:nesso-main}, column \emph{Media
         Totale}.}
\label{fig:nesso-all}
\end{center}
...
\begin{center}
\includegraphics[width=\linewidth]{images/italian-nesso.png}
\captionof{figure}{Italian-language benchmark results. Data from Table~\ref{tab:nesso-main}, columns
         IFEval IT, ARC\_IT, HS\_IT, MMLU\_IT.}
\label{fig:italian-nesso}
\end{center}
\par\medskip
...
\begin{center}
\includegraphics[width=\linewidth]{images/english_nesso.png}
\captionof{figure}{English-language benchmark results. Data from Table~\ref{tab:nesso-main}, columns
         IFEval EN, ARC\_EN, HS\_EN, MMLU\_EN.}
\label{fig:english-nesso}
\end{center}
\par\medskip
...
\par\medskip
\begin{center}
\captionof{table}{Open-Nesso-0.4B evaluation (Italian benchmarks).}
\label{tab:open-nesso}
\begin{tabular}{lcccc}
\toprule
Model & mmlu\_it & arc\_it & hellaswag\_it & Media \\
...
\par\medskip
\begin{center}
\captionof{table}{EVALITA task-level comparison: Zagreus-0.4B-ita-base
         (see Table~\ref{tab:evalita-fbk}) vs.\ Open-Zagreus-0.4B (SFT on
         \cite{openitaliandata}).}
\label{tab:evalita-comparison}
\resizebox{\linewidth}{!}{%
...
\begin{center}
\includegraphics[width=\linewidth]{images/evalita_comparison.png}
\captionof{figure}{Per-task EVALITA comparison: base model (Zagreus-0.4B-ita) vs.\
         Open-Zagreus-0.4B. Data from Table~\ref{tab:evalita-comparison}.}
\label{fig:evalita-comparison}
\end{center}
\par\medskip
